<x-guest-layout>
    <x-authentication-card>
        ...
        <x-auth.title>
            ...
            <x-slot name="description">{{ __('Зареєструйтеся в Open Health') }}</x-slot>
        </x-auth.title>
        <!-- ====== Forms Section Start -->
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                ...
                @error('password') <span class="flex items-center font-medium tracking-wide text-danger text-xs mt-1 ml-1">{{ $message }}</span>@enderror
            </div>
