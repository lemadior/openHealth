...
//        $request['uuid'] = 'f13ab4b7-1167-4215-9fb3-2116b775ddb1';
        ...
        if (!empty($request) ){
            ...
            $this->legalEntity->save();
            $user = $this->createUser($request['urgent']);
    ...
    {
        $auth = Auth::user();
        $auth->legalEntity()->associate($this->legalEntity);
        $auth->save();

        // Check if the authenticated user's email matches the owner's email
        if ($auth->email === $this->legal_entity_form->owner['email']) {
            $auth->update([
                'email' => $this->legal_entity_form->owner['email'] ?? null,
                'client_id' => $data['security']['client_id'] ?? null,
                'secret_key' => $data['security']['client_id'] ?? '',
            ]);
            return $auth; // Return the authenticated user if the update is done
        }
        ...
        $user = User::create([
            ...
            'client_id' => $data['security']['client_id'] ?? null,
            'secret_key' => $data['security']['client_secret'] ?? '',
        ]);
