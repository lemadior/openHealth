...
class HealthcareServiceShow extends HealthcareServiceComponent
{
    ...
    {
        // $this->formService = new HealthCareFormRequest();
        if (!$division || !$healthcareService) {
            abort(404);
        }

        $this->speciality_type = dictionary()->getDictionary('SPECIALITY_TYPE', false)->toArrayRecursive();

        $this->license_show = false;

        // if ($healthcareService) {
            $this->formService->setHelathcareService($healthcareService->toArray());
        // }

        // Set the division model instance
        $this->dictionaries = [
            'show' => [],
            'modal' => []
        ];
    ...
    public function show(HealthcareService $healthcareService) {
        $this->formService->setHelathcareService($healthcareService->toArray());
    ...
    public function render(): View
    {
        ...
        return view('livewire.healthcare-service.healthcare-service-show', compact(['healthcareServices']));
    }
}
