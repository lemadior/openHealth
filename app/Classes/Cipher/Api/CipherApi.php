...
class CipherApi
{
    ...
    ): array|string
    {
        ...
        try {
            ...
            $this->uploadFile();
            $this->verifyWithFileContainer($taxId, $edrpou, $initiator);
            $this->createKeyp();
            $this->getKeypCreator();
            return $this->getKeyp();
     ...
     * @return void
     */
    public function verifyWithFileContainer(string $taxId, ?string $edrpou, string $initiator): void
    {
        ...
        $isBusinessKey = !empty($inKeyEdrpou) || $edrpou === $taxId;
        ...
        if ($initiator === self::SIGNATORY_INITIATOR_BUSINESS) {
            ...
            if ($inKeyEdrpou !== $taxId && $edrpou !== $taxId) {
                ErrorHandler::throwError(
                    [
